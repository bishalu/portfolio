  ...
  return (
    <div className="pointer-events-none absolute inset-0 h-full w-full overflow-hidden">
      ...
      {cells.map((cell) => (
        <div
          key={cell.id}
          className="animate-pulse-data absolute h-1 w-1 rounded-full bg-white opacity-0"
          style={{
            top: cell.top,
            left: cell.left,
      ...
      if (depth < 3 && Math.random() < 0.5) {
        ...
        segments.push(
          ...generateBolt(
            newMidX,
            newMidY,
            newMidX + Math.cos(finalAngle) * branchLength,
            newMidY + Math.sin(finalAngle) * branchLength,
            depth + 2,
          ),
        )
      }
  ...
  return (
    <div className="pointer-events-none absolute inset-0 h-full w-full overflow-hidden">
      {embers.map((ember) => (
        <div
          key={ember.id}
          className="animate-rise absolute rounded-full bg-gradient-to-t from-[rgba(232,168,124,0.1)] to-transparent"
          style={{
            left: ember.left,
            bottom: '-20%',
            width: ember.size,

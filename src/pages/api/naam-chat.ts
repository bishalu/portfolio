...
/**
 * The outer wall, and it is now only a wall. The cut that was actually being
 * seen happens earlier, in coerceModelReply at NAAM_MAX_REPLY_CHARS — this one
 * at 1,200 had never fired. Both now end at a sentence through the same helper.
 */
function trimReply(text: string): string {
  return trimToSentence(text, MAX_REPLY_CHARS)
}
const POOL_MAX = 60
const ASK_MAX = 400
